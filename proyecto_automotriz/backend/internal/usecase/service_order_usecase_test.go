...
func TestOpenCreatesTheOrderInReceivedStatus(t *testing.T) {
	...
	useCase := usecase.NewServiceOrderUseCase(
		orders, newFakeVehicleRepository("vehicle-1"), &fakeAssignmentRepository{}, sequentialID(), fixedClock(),
	)
...
func TestOpenRejectsAnUnknownVehicle(t *testing.T) {
	useCase := usecase.NewServiceOrderUseCase(
		newFakeOrderRepository(), newFakeVehicleRepository("vehicle-1"), &fakeAssignmentRepository{}, sequentialID(), fixedClock(),
	)
...
func TestAdvanceRejectsAnOutOfLifecycleMoveAndWritesNothing(t *testing.T) {
	...
	_, err := useCase.Advance(context.Background(), "order-1", domain.StatusReady, "user-1")
...
func TestAdvanceWritesTheTransitionRecordWithItsAuthor(t *testing.T) {
	...
	if _, err := useCase.Advance(context.Background(), "order-1", domain.StatusInDiagnosis, "user-1"); err != nil {
...
func TestAdvanceToDeliveredReleasesTheTechnician(t *testing.T) {
	...
	useCase := usecase.NewServiceOrderUseCase(
		orders, newFakeVehicleRepository("vehicle-1"), assignments, sequentialID(), fixedClock(),
	)

	if _, err := useCase.Advance(context.Background(), "order-1", domain.StatusDelivered, "user-1"); err != nil {
