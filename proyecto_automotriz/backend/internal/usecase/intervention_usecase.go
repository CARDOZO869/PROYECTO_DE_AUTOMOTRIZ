...
type InterventionRepository interface {
	...
	SaveWithStatus(ctx context.Context, intervention domain.Intervention, order domain.ServiceOrder, transition domain.StatusTransition) error
...
func (i InterventionUseCase) Register(
	...
	if order.Status != domain.StatusInDiagnosis && order.Status != domain.StatusInRepair {
		return domain.Intervention{}, fmt.Errorf("%w: an intervention can only be recorded in IN_DIAGNOSIS or IN_REPAIR", domain.ErrInvalidTransition)
	}
	...
	if order.Status == domain.StatusInDiagnosis {
		transition, err = order.MoveTo(domain.StatusInRepair, i.newID(), actorUserID, performedAt)
		if err != nil {
			return domain.Intervention{}, err
		}
	}
	if err := i.intervention.SaveWithStatus(ctx, intervention, order, transition); err != nil {
		return domain.Intervention{}, err
	}
