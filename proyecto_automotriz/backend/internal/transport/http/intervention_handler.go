...
func NewInterventionHandlerWithAccess(intervention usecase.InterventionUseCase, assignment usecase.AssignmentUseCase, technician usecase.TechnicianUseCase) InterventionHandler {
	return InterventionHandler{intervention: intervention, assignment: assignment, technician: technician, accessConfigured: true}
}
...
func (h InterventionHandler) Register(writer http.ResponseWriter, request *http.Request) {
	identity, err := requireTechnician(request.Context())
	if err != nil {
		failure(writer, err)
		return
	}
