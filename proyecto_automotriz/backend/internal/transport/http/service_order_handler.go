...
func NewServiceOrderHandlerWithAccess(order usecase.ServiceOrderUseCase, assignment usecase.AssignmentUseCase, technician usecase.TechnicianUseCase) ServiceOrderHandler {
	return ServiceOrderHandler{order: order, assignment: assignment, technician: technician, accessConfigured: true}
}
...
func (h ServiceOrderHandler) List(writer http.ResponseWriter, request *http.Request) {
	...
	var listed []usecase.ServiceOrderSummary
	if identity.Role == domain.RoleAdministrator {
		listed, err = h.order.List(request.Context(), status)
	} else {
		listed, err = h.order.ListForTechnician(request.Context(), status, identity.UserID)
	}
	if err != nil {
		failure(writer, err)
		return
...
func (h ServiceOrderHandler) Find(writer http.ResponseWriter, request *http.Request) {
	...
	if h.accessConfigured {
		if err := requireOrderAccess(request.Context(), request.PathValue("serviceOrderId"), identity, h.assignment, h.technician); err != nil {
			failure(writer, err)
			return
		}
	}
	order, err := h.order.Find(request.Context(), request.PathValue("serviceOrderId"))
	if err != nil {
		failure(writer, err)
		return
	}
...
func (h ServiceOrderHandler) Advance(writer http.ResponseWriter, request *http.Request) {
	identity, err := requireAdministrator(request.Context())
	if err != nil {
		failure(writer, err)
		return
	}
