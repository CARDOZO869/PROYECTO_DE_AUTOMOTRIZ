...
func NewServer(dependency Dependency) *http.Server {
	...
	technicianUseCase := usecase.NewTechnicianUseCase(technicianRepository)
	technicianHandler := NewTechnicianHandler(technicianUseCase)
	assignmentUseCase := usecase.NewAssignmentUseCase(assignmentRepository, orderRepository, technicianRepository, newID, now)
	orderUseCase := usecase.NewServiceOrderUseCase(orderRepository, vehicleRepository, assignmentRepository, newID, now)
	orderHandler := NewServiceOrderHandlerWithAccess(orderUseCase, assignmentUseCase, technicianUseCase)
	assignmentHandler := NewAssignmentHandler(assignmentUseCase)
	diagnosticHandler := NewDiagnosticHandlerWithAccess(usecase.NewDiagnosticUseCase(
		diagnosticRepository, orderRepository, assignmentRepository, technicianRepository, newID, now,
	), assignmentUseCase, technicianUseCase)
	interventionHandler := NewInterventionHandlerWithAccess(usecase.NewInterventionUseCase(
