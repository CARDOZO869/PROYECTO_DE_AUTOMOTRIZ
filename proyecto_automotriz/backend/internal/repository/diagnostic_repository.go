...
// SaveWithStatus stores the diagnostic and its first status transition atomically.
func (r DiagnosticRepository) SaveWithStatus(ctx context.Context, diagnostic domain.Diagnostic, order domain.ServiceOrder, transition domain.StatusTransition) error {
	queryCtx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()
	...
	if _, err := tx.ExecContext(queryCtx, "INSERT INTO diagnostic ("+diagnosticColumn+") VALUES (?, ?, ?, ?, ?, ?)", diagnostic.ID, diagnostic.ServiceOrderID, diagnostic.TechnicianID, diagnostic.Finding, diagnostic.ComponentToRepair, diagnostic.CreatedAt); err != nil {
		return translate(err)
	}
