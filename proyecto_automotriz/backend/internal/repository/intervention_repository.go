...
// SaveWithStatus stores the intervention, its parts and its status transition atomically.
func (r InterventionRepository) SaveWithStatus(ctx context.Context, intervention domain.Intervention, order domain.ServiceOrder, transition domain.StatusTransition) error {
	queryCtx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()
	...
	if transition.ID != "" {
		if _, err := tx.ExecContext(queryCtx, "UPDATE service_order SET status = ?, updated_at = ? WHERE id = ?", string(order.Status), order.UpdatedAt, order.ID); err != nil {
			return translate(err)
		}
		if _, err := tx.ExecContext(queryCtx, "INSERT INTO status_transition (id, service_order_id, from_status, to_status, changed_by_user_id, changed_at) VALUES (?, ?, ?, ?, ?, ?)", transition.ID, transition.ServiceOrderID, string(transition.FromStatus), string(transition.ToStatus), transition.ChangedByUserID, transition.ChangedAt); err != nil {
			return translate(err)
		}
	}
	if _, err := tx.ExecContext(queryCtx, "INSERT INTO intervention ("+interventionColumn+") VALUES (?, ?, ?, ?, ?, ?, ?)", intervention.ID, intervention.ServiceOrderID, intervention.TechnicianID, intervention.Description, intervention.LaborHourCount, intervention.PerformedAt, intervention.CreatedAt); err != nil {
		return translate(err)
	}
	for _, part := range intervention.Part {
		if _, err := tx.ExecContext(queryCtx, "INSERT INTO part_usage (id, intervention_id, part_name, quantity, created_at) VALUES (?, ?, ?, ?, ?)", part.ID, intervention.ID, part.PartName, part.Quantity, part.CreatedAt); err != nil {
			return translate(err)
		}
	}
