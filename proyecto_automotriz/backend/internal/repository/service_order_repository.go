...
// List returns every order, optionally filtered by status.
func (r ServiceOrderRepository) List(ctx context.Context, status string) ([]usecase.ServiceOrderSummary, error) {
	...
	return r.listSummaries(ctx, serviceOrderSummarySelect+where+" ORDER BY so.received_at DESC", args...)
}

// ListForTechnician limits the result to the orders actively assigned to the signed-in technician.
func (r ServiceOrderRepository) ListForTechnician(ctx context.Context, status, userID string) ([]usecase.ServiceOrderSummary, error) {
	where := " WHERE a.is_active = 1 AND u.id = ?"
	args := []any{userID}
	if status != "" {
		where += " AND so.status = ?"
		args = append(args, status)
...
func (r ServiceOrderRepository) listSummaries(ctx context.Context, query string, args ...any) ([]usecase.ServiceOrderSummary, error) {
	queryCtx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()
	rows, err := r.database.QueryContext(queryCtx, query, args...)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&summary.Order.ID, &summary.Order.OrderNumber, &summary.Order.VehicleID, &summary.Order.ReportedFailure, &statusValue, &summary.Order.ReceivedAt, &summary.Order.CreatedAt, &summary.Order.UpdatedAt, &summary.VehiclePlate, &summary.TechnicianName); err != nil {
			return nil, translate(err)
		}
...
func (r ServiceOrderRepository) ListTransition(ctx context.Context, serviceOrderID string) ([]domain.StatusTransition, error) {
	...
	rows, err := r.database.QueryContext(
		...
		"SELECT st.id, st.service_order_id, st.from_status, st.to_status, st.changed_by_user_id, COALESCE(u.full_name, ''), st.changed_at "+
			"FROM status_transition st JOIN `user` u ON u.id = st.changed_by_user_id WHERE st.service_order_id = ? ORDER BY st.changed_at",
		serviceOrderID,
	)
	if err != nil {
		return nil, translate(err)
...
func (r ServiceOrderRepository) NextOrderNumber(ctx context.Context) (string, error) {
	...
	var next int
	if err := r.database.QueryRowContext(queryCtx, "SELECT COALESCE(MAX(CAST(SUBSTRING(order_number, 4) AS UNSIGNED)), 0) + 1 FROM service_order WHERE order_number LIKE 'OS-%'").Scan(&next); err != nil {
		return "", translate(err)
	}
	return fmt.Sprintf("OS-%04d", next), nil
}
