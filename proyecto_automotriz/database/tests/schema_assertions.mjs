...
// positive quantity rules and the one active assignment per technician and per order rule.
const TABLE_NAME = [
  'user',
  'customer',
  'vehicle',
...
const UNIQUE_INDEX = [
  ...
  ['assignment', 'uq_assignment_active_order_marker'],
];
...
export function runAssertions(context) {
  ...
  const passwordCosts = queryScalar(
    'SELECT COUNT(*) FROM ' + USER_TABLE + " WHERE id IN ('22222222-2222-4222-8222-222222222222', '33333333-3333-4333-8333-333333333333') AND password_hash LIKE '$2a$12$%'",
  );
  assert(passwordCosts === '2', 'technician bootstrap passwords use bcrypt cost 12');
  checks += 1;
  ...
  runSql(
    "INSERT INTO assignment (id, service_order_id, technician_id, is_active, active_marker, active_order_marker, assigned_at) VALUES ('60000000-0000-4000-8000-000000000001', '"
      + ORDER_ONE_ID + "', '" + TECHNICIAN_ONE_ID + "', 1, '" + TECHNICIAN_ONE_ID + "', '" + ORDER_ONE_ID + "', NOW());",
  );
  checks += 1;

  assert(
    expectSqlError(
      "INSERT INTO assignment (id, service_order_id, technician_id, is_active, active_marker, active_order_marker, assigned_at) VALUES ('60000000-0000-4000-8000-000000000002', '"
        + ORDER_TWO_ID + "', '" + TECHNICIAN_ONE_ID + "', 1, '" + TECHNICIAN_ONE_ID + "', '" + ORDER_TWO_ID + "', NOW());",
    ),
  ...
  runSql(
    "INSERT INTO assignment (id, service_order_id, technician_id, is_active, active_marker, active_order_marker, assigned_at) VALUES ('60000000-0000-4000-8000-000000000003', '"
      + ORDER_TWO_ID + "', '" + TECHNICIAN_TWO_ID + "', 1, '" + TECHNICIAN_TWO_ID + "', '" + ORDER_TWO_ID + "', NOW());",
  );
  checks += 1;

  runSql(
    "UPDATE assignment SET is_active = 0, active_marker = NULL, active_order_marker = NULL, released_at = NOW() WHERE id = '60000000-0000-4000-8000-000000000001';",
  );

  assert(
    expectSqlError(
      "INSERT INTO assignment (id, service_order_id, technician_id, is_active, active_marker, active_order_marker, assigned_at) VALUES ('60000000-0000-4000-8000-000000000004', '"
        + ORDER_TWO_ID + "', '" + TECHNICIAN_ONE_ID + "', 1, '" + TECHNICIAN_ONE_ID + "', '" + ORDER_TWO_ID + "', NOW());",
    ),
    'a second active assignment for the same service order is rejected',
  );
  checks += 1;

  assert(
    expectSqlError(
      "INSERT INTO assignment (id, service_order_id, technician_id, is_active, active_marker, active_order_marker, assigned_at) VALUES ('60000000-0000-4000-8000-000000000005', '"
        + ORDER_THREE_ID + "', '" + TECHNICIAN_ONE_ID + "', 1, NULL, '" + ORDER_THREE_ID + "', NOW());",
    ),
    'an active assignment without its technician marker is rejected',
  );
  checks += 1;
