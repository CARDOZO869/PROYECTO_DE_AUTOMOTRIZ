...
export function LoginPage() {
  ...
  if (session) {
    return <Navigate to={session.role === 'ADMINISTRATOR' ? '/dashboard' : '/service-orders'} replace />;
  }
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      navigate(created.role === 'ADMINISTRATOR' ? '/dashboard' : '/service-orders', { replace: true });
    } catch (failure) {
      setPassword('');
      setError(
